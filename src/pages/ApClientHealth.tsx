...
export default function ApClientHealth() {
  ...
  useEffect(() => {
    fetch('/eero-fleet/data/ap_client_exec_summary.json').then(r=>r.json()).then(setSummaries).catch(()=>{});
    fetch('/eero-fleet/data/jira_ap_client.json').then(r=>r.json()).then(d=>{
      setJiras(d.tickets||[]);
      setJiraStatus('done');
  ...
  return (
    <div style={{maxWidth:'98%',margin:'0 auto',padding:'20px'}}>
      <h1 style={{color:'#58a6ff',fontSize:'20px'}}>📡 AP-Client Health Check</h1>
  ...
  return (
    <div>
      ...
      <div style={{background:'#161b22',border:'2px solid #58a6ff',borderRadius:'8px',padding:'12px',marginTop:'8px'}}>
        ...
        {(!latest.fleet_regression || latest.fleet_regression.length === 0) && (
          <div style={{marginTop:'8px',color:'#3fb950',fontSize:'11px'}}>✅ AP-Client healthy</div>
        )}
      </div>
    </div>
  );
