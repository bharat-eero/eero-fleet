...
function harmonyUrl(url) {
  ...
  if (url.startsWith('/api/exec-summary') && !url.includes('ap-client') && !url.includes('acs')) return '/data/exec_summary.json';
  if (url.startsWith('/api/ap-client-exec-summary')) return '/data/ap_client_exec_summary.json';
  if (url.startsWith('/api/mesh-jiras')) return '/data/jira_mesh.json';
  ...
  if (url.startsWith('/fleet-metric/')) return '/data/fleet_health/' + url.substring('/fleet-metric/'.length);
  return url;
}
...
function buildDashboardLink(r){
  ...
  if(!name) return null;
  if(IS_HARMONY) return '/data/fleet_health/'+name+'/dashboard.html';
  return '/fleet-metric/'+name+'/dashboard.html';
}
...
try {
  const cached = JSON.parse(localStorage.getItem('meshJirasCache')||'null');
  if(cached && cached.tickets) { _meshJiras = cached.tickets; }
} catch(e){}
let _lastCutoff='';
...
function renderOneExecSummary(data, isLatest, cat){
  ...
  function jiraBtn(m){
    ...
    // Show old matches
    const oldMatches=_meshJiras.filter(j=>j.metric===m.name && j.key!==(t&&t.key) && (j.status==='Done'||j.status==='Closed'||j.status==="Won't Do"));
  ...
  const oldJiras=_meshJiras.filter(t=>{
...
async function loadAll(){
  ...
  const jiraPromise = api('GET','/api/mesh-jiras').catch(function(e){console.error('jira',e);return {ok:false}});
...
function _renderAll(list, jr, es){
  ...
  if (IS_HARMONY && list.length && list[0].pipeline && !list[0].run) {
    list = list.map(r => ({
      category: r.pipeline || 'fleet_health',
      run: r.name,
      path: r.path,
  ...
  const fh=list.filter(r=>r.category==='fleet_health');
  ...
  // Jiras
  if(jr.tickets){
    _meshJiras=jr.tickets||[];
    localStorage.setItem('meshJirasCache', JSON.stringify({tickets:_meshJiras, ts:new Date().toISOString()}));
  }
  // Show sync status with cache time
  const jiraStatus = document.getElementById('jira-sync-status');
  if(jiraStatus){
    const cachedTs = (JSON.parse(localStorage.getItem('meshJirasCache')||'{}').ts||'').substring(0,16).replace('T',' ');
    ...
    else if(jr.tickets){ jiraStatus.innerHTML='<span style="color:#66bb6a;font-size:18px">✅ '+_meshJiras.length+' Jira tickets synced</span>'; setTimeout(()=>{jiraStatus.style.transition='opacity 1s';jiraStatus.style.opacity='0.4'},3000); }
...
function jiraRow(t){
  ...
  const isAI=(t.labels||[]).includes('eero-ai-mesh-metric');
  ...
  const lbls=(t.labels||[]).map(l=>'<span style="background:'+(l==='eero-ai-mesh-metric'?'#4fc3f733':'#333')+';color:'+(l==='eero-ai-mesh-metric'?'#ffeb3b':'inherit')+';'+(l==='eero-ai-mesh-metric'?'font-weight:bold;text-shadow:0 0 6px #ffeb3b,0 0 12px #ffeb3b88;':'')+'padding:1px 4px;border-radius:2px;font-size:9px">'+l+'</span>').join(' ');
...
function renderIssueSummary(cutoff){
  ...
  if(!el || !_meshJiras.length) { if(el) el.innerHTML=''; return; }
  ...
  h+='<b style="font-size:15px">'+(frozenDate?'📌 Current Run Summary':'📋 Mesh metrics weekly summary')+'</b>';
  ...
  const jiraLabels='project%3DCONN%20AND%20labels%20in%20(mesh-metrics-review%2Ceero-ai-mesh-metric)';
...
async function refreshRun(runName){
  const runDir = '/home/bharatmk/myeeroai_results/_shared/fleet_health/' + runName;
  const v1 = '/home/bharatmk/bharat_sandbox/myeeroai_web_v1';
  const cmd = 'python3 '+v1+'/fleet_health/07_refresh.py '+runDir;
...
async function refreshJiras(){
  ...
  try{
    ...
    if(jr.tickets) _meshJiras=jr.tickets||[];
...
async function switchLogTab(tab){
  ...
  try{
    ...
    if(tab==='runlog'){
      ...
      const resp=await fetch('/fleet-metric/'+run.name+'/analysis/fetch_summary.json');
...
async function runSelected(){
  ...
  for(const cat of cats){
    ...
    const res=await api('POST','/api/run',{task:'fleet_health',params:{env:c.env,group_ids:c.gids,investigate:inv,days:days,metric:metric,no_cache:noCache,pyspark:pyspark},user:localStorage.getItem('user')||'anonymous'});
...
async function showProcesses(){
  ...
  if(!msg) msg = 'No kiro-cli or fleet_health processes running.';
...
const SCHED_PIPELINE='fleet_health';
